...
- Scalar configuration defaults (`config/scalar.ts` — includes `withDefaultFonts: false` so Inter / JetBrains Mono are not loaded from `fonts.scalar.com`; families remapped to Codex stacks in `main.css`)
