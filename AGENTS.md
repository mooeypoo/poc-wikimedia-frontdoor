...
- `useExplorerModuleRailPlacement()` / `useExplorerModuleRailInlineEndpointScrollCap(…)` — **legacy** custom module rail only (not mounted while `EXPLORER_USE_INTERNAL_SCALAR_SIDEBAR` is true; PR #40)
...
- `useScalarClientModalBackgroundScrollLock(scalarShellRef, scalarInterface)` — Test Request UI exploration: on open, scrolls shell into view and toggles `--client-modal-open` (natural-height modal, full-shell exit, 40px gutter, 4px-inset close control, exploratory 4px radius); sets `--fd-explorer-test-request-shell-block-size` from dialog **`scrollHeight` + gutter** (`ResizeObserver`, deadband from `config/explorerSurfaces.ts`; skip collapsed heights &lt; gutter); shell **`block-size` / `overflow-block: clip` must be scoped** in `explorer/[...view].vue` (unscoped loses to `[data-v-*]`); overlay/container **`height: auto`** in `explorer-codex-overrides.css`; clears clamp when `scalarInterface` is nulled/replaced (spec remount — page must null the handle on `scalarReferenceKey` change); content above shell + footer stay reachable; while open, `inert` on the dialog when pointerdown is outside the Scalar shell so header preferences `CdxPopover` is not focus-trap-dismissed; shell pointerdown clears `inert` and retargets `.app-exit-button` so close still works after a theme change
...
- [ ] Scalar Test Request UI exploration: dialog **above** Scalar endpoint sidebar (`z-index: 10000`); grows with content (no `90svh` / `h-dvh` / viewport cap); **`.scalar-app-exit` covers the full shell**; dialog gutter **`--spacing-250` (40px)**; **`.app-exit-button`** outside the dialog with **4px** (`--spacing-25`) inset at block-start / inline-end from `.scalar.scalar-app` (light and dark; keep Scalar stock `p-2` size); header preferences `CdxPopover` stays open while Test Request is open (dialog `inert` when interacting outside the Scalar shell; close must still work after a theme change — shell pointerdown clears `inert` / retargets `.app-exit-button`); dialog radius **`--fd-explorer-controls-surface-border-radius` (4px)** with dialog **`overflow: hidden`** (so square `.bg-b-1` children do not hide the radius); while open shell **block-size = dialog `scrollHeight` + gutter** (`EXPLORER_TEST_REQUEST_*` in `config/explorerSurfaces.ts`; `ResizeObserver` deadband; ignore heights &lt; gutter); **scoped** **`overflow-block: clip`** in `explorer/[...view].vue` (must beat `[data-v-*]`); overlay/container **`height: auto`**; on module/instance remount null **`scalarInterface`** so clamp clears (Scalar often skips `ui:close` — do not leave ~80px shell); no scroll into specs; page scrolled so shell is in view on open; **`.frontdoor-shell__body-scroll` stays scrollable** for tall sandboxes / content above / footer; CSS under `--client-modal-open`; **no** body-wide MutationObserver (freezes mount)
...
- Picker menu labels use BiDi isolation (`isolatePickerLabel()`); external module names in project controls use isolation / `<bdi>` as documented
...
- Picker menu labels and module descriptions use BiDi isolation (`isolatePickerLabel()`); REST API module select uses native Codex menu hover, keyboard highlight, and selected styling (no custom `.cdx-menu-item` state overrides on the explorer page); beta/internal modules show **label-only** warning InfoChips beside the name with version-only supporting text (Codex exception #14); project controls module row uses **`column-gap: var(--spacing-150)`** (24px) between REST API module select and Opt-in fieldset; Scalar’s native sidebar lists endpoints for the loaded module (no custom rail BiDi / row chrome in product UX)
