...
- `useExplorerModuleSelect(visibleModules, …)` — REST API module `CdxSelect` menu items (`label`, `supportingText`, `description`) sorted alphabetically by `headingTitle`, `menu-config`, `default-label`, and selection bridge for project controls
...
- REST API module select menu options are sorted alphabetically by `headingTitle` (after opt-in filter); default selection remains the first healthy module in discovery order
