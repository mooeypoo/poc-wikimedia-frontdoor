...
- [ ] **Navigation card** (`NavigationCard` / `NavigationCardGrid`): title, description, supporting-text, and chip labels in `<bdi>`; no banana keys for card copy; first-party CSS uses logical properties; hover border `--border-color-subtle` on linked cards; whole-card click via stretched link; when `supporting-text` + `url` are set, supporting-text is a progressive link to the same destination (external icon on supporting-text for off-platform URLs; title trailing icon omitted); supporting-text bottom-aligned in equal-height grid rows; without supporting-text, title trailing icon for off-platform URLs; omit `url` for intentionally non-clickable cards; description may include inline links; ProseA external icons suppressed in card descriptions; Get started / Build for communities / Use wiki content / About Wikimedia cards sit in `:::navigation-card-grid` (equal-height rows); internal card/`sectionNavigation` `href`s have matching `content/<locale>/…` Markdown (e.g. `/get-started/on-wiki` → `on-wiki.md`) so destinations do not 404
