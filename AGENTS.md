...
- `useScalarClientModalBackgroundScrollLock(scalarShellRef, scalarInterface)` — Test Request UI exploration: on open, scrolls shell into view and toggles CSS (natural-height modal, full-shell exit, 40px gutter, 4px-inset close control, exploratory 4px radius); while open, `inert` on the dialog when pointerdown is outside the Scalar shell so header preferences `CdxPopover` is not focus-trap-dismissed; shell pointerdown clears `inert` and retargets `.app-exit-button` so close still works after a theme change; page scroll stays free; never `overflow: hidden` on the shell
