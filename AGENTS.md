...
- [ ] Header **color theme preferences**: settings gear (or collapsed utility **Settings**) opens `CdxPopover` with `CdxField` (`is-fieldset`) + `CdxRadio` Light / Dark / System default (`light` / `dark` / `auto` via `COLOR_THEME_PREFERENCE_OPTIONS` in `config/colorMode.ts`); labels via banana (`color-mode-group-label`, `color-mode-*-label`); selection calls `useColorMode().setMode` only (no duplicate theme logic); popover **stays open** after select; **no** title/close chrome (dismiss via outside click / Escape); collapsed mode may anchor to the overflow `CdxMenuButton`; first-party CSS uses logical properties; inherits interface `dir`
