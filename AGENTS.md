...
**Read [`DESIGN_REQUIREMENTS.md`](DESIGN_REQUIREMENTS.md) before changing** anything that affects what users see or how they move through the shell: `app/layouts/`, `app/components/shared/`, shell layout CSS (`app/assets/css/page-grid.css`, `app/assets/css/shell-start-nav-reveal.css`, `app/assets/css/shell-start-nav-scroll.css`, `app/assets/css/shell-collapsed-nav-menu.css`, `app/assets/css/shell-end-panel-nav.css`), content-page visual patterns in `app/assets/css/main.css` (e.g. `.fd-content-page` typography and `h2` section spacing), explorer UI components, or site-wide visual patterns. Implement to match recorded decisions there (e.g. desktop **4 \| 16 \| 4** grid, end-panel nav aligned via `useEndPanelNavAlign`, content-page `h2` **`--spacing-250`**) unless the user explicitly requests a design change.
...
- Navigation card allowlisted brand title logos for MDC (`config/navigationCardTitleLogos.ts` — `gerrit` / `github` / `gitlab`)
...
- Scalar configuration defaults (`config/scalar.ts` — includes `hideClientButton: true` so the Open API Client control is off everywhere; prefer schema flags over CSS `display: none` on Scalar Tailwind classes)
...
**Brand title logos (exception):** When the card title is a company mark instead of text (Browse repositories on `/get-started/by-language`), use allowlisted `title-logo="gerrit|github|gitlab"` from `config/navigationCardTitleLogos.ts` — monochrome SVG at `--size-icon-medium`, `currentColor` / `--color-base` (not progressive). Keep writer `supporting-text`. Do not invent new brand ids without adding them to that config.
...
- `usePrimaryNavigationTab()` — active primary nav tab id (`apis` stays selected on `/apis`, `/apis/…`, `/explorer`, and `/explorer/…`)
...
- `useScalarClientWriteEndpointWarnings(scalarInterface)` — injects the write-request production warning into the Scalar Test Request modal
...
- [ ] Scalar chrome: **`hideClientButton: true`** in `config/scalar.ts` (no Open API Client control); do not hide via CSS on Scalar Tailwind classes; **Test Request** stays enabled unless product asks otherwise
- [ ] Scalar Test Request modal write-request **`CdxMessage`** uses banana-i18n; production wiki display name and mocked test-wiki link label (`Test Wikipedia` / `Test Wikidata` / `Test Wikimedia Commons`) are wrapped in `<bdi>`; the `$2` link is progressive-styled and currently a no-op until test wikis are discoverable
...
- Write-request Test Request modal: **`CdxMessage`** appears below the address bar for POST/PUT/PATCH/DELETE when a test wiki is mapped; copy names the production wiki and a mocked link to the test wiki display name; requests are not rewritten away from production
