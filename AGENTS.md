...
**Documented exception — Test Request shell clamp:** While Test Request is open, scoped styles on `.explorer-page__scalar-shell--client-modal-open` set **`overflow-block: clip`** and **`block-size`** from **`--fd-explorer-test-request-shell-block-size`** so page scroll cannot continue into specs. Unscoped `explorer-codex-overrides.css` cannot beat Vue `[data-v-*]` for those properties — keep the clamp in `app/pages/explorer/[[view]].vue`. Overlay/container stay **`height: auto`** (not shell `100%`). See `ARCHITECTURE.md` → Scalar Test Request modal (natural height).
