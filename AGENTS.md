...
**Documented exception — header brand focus outline:** `ShellHeaderBrand` suppresses `:focus` / `:focus-visible` / `:active` / router-active **outline** so the mark + wordmark stay visually quiet. `aria-label` from `app-title` remains. Do not reintroduce a focus ring without updating `DESIGN_REQUIREMENTS.md` → Brand logo. See `ARCHITECTURE.md` → Codex exceptions (shell chrome) #6.

**Documented exception — landing band `linear-gradient` direction:** APIs / join bands use physical **`to bottom`**. `linear-gradient()` has no shipped logical side keywords (`to block-end` is invalid and drops the declaration). Vertical band fills are writing-mode stable here. See `LandingBand.vue` / `landing-page.css`.
...
- Use `CdxButton` for inline call-to-action buttons. Markdown CTAs use `AppButton` (`::app-button`), which wraps progressive primary `CdxButton` (avoids shell prose-link colour washing out inverted label text on `<a>`-styled buttons).
...
- **Approved exception — catalog InfoChips are label-only:** Optional `CdxInfoChip` rows on `NavigationCard` keep stock Codex **status colours** but hide status icons. Codex forces icons on `warning` / `error` / `success` and ignores a null `icon` prop for those statuses, so `NavigationCard` hides `.cdx-info-chip__icon--vue` under `.navigation-card__chips`. Do not re-enable icons or invent a second chip component. Landing Coolest Tool chips use `chips="award:…"` (star + purple100/600 from `LANDING_AWARD_CHIP`; dark inverted via `-light`/`-dark` CSS vars — see `ARCHITECTURE.md` → Navigation card → Info chips).
- Platform home (`content/en/index.md`, `.fd-landing-page`): use `LandingHero` / `LandingBand` / `LandingSection` / `LandingApiDemo` / `LandingSectionCta` for Figma Latest (1179:23177) surfaces. Community app screenshots and award-chip colours live in `config/landingSurfaces.ts` / `public/images/landing/` — do not invent missing assets. API preview thumbs are preloaded from `app/pages/index.vue` (keep Codex `CdxCard` / `CdxThumbnail`). See `DESIGN_REQUIREMENTS.md` → Platform landing / home.
...
- [ ] Header brand (`ShellHeaderBrand`): banana wordmark + `aria-label` from `app-title`; mark via `WikimediaLogoMark` / `currentColor`; **no** `:focus` / `:focus-visible` / `:active` / router-active outline (Codex exception #6); first-party CSS uses logical properties
