...
|----------|-------------|
| **`AGENTS.md`** (this file) | Non-negotiable implementation rules — always follow; **Navigation card authoring playbook** (internal vs external card styles + **link-only** `:::link-row` for docs automation) |
...
| **[`docs/content-authoring-guide.md`](docs/content-authoring-guide.md)** | Markdown / MDC authoring, including navigation-card and link-row examples |
...
**Read [`DESIGN_REQUIREMENTS.md`](DESIGN_REQUIREMENTS.md) before changing** anything that affects what users see or how they move through the shell: `app/layouts/`, `app/components/shared/`, shell layout CSS (`app/assets/css/page-grid.css`, `app/assets/css/shell-start-nav-reveal.css`, `app/assets/css/shell-start-nav-scroll.css`, `app/assets/css/shell-end-panel-nav.css`), content-page visual patterns in `app/assets/css/main.css` (e.g. `.fd-content-page` typography and `h2` / `h3` section spacing), platform landing surfaces in `app/assets/css/landing-page.css` and `app/components/content/Landing*.vue`, explorer UI components, or site-wide visual patterns. Implement to match recorded decisions there (e.g. desktop **4 \| 16 \| 4** grid, explorer module rail aligned via `useEndPanelNavAlign`, documentation on-this-page TOC via CSS sticky — not that composable — content-page `h2` **`--spacing-250`**, site-wide `h3` **`--spacing-200`**, landing content measure **1000px** via `config/landingSurfaces.ts`) unless the user explicitly requests a design change.
...
- Community Explorer uses Scalar’s native operation sidebar (`EXPLORER_USE_INTERNAL_SCALAR_SIDEBAR` in `config/explorerInternalSidebarExperiment.ts` → `showSidebar` in `config/scalar.ts`; PR #40). The custom module rail is not product UX (code retained for rollback only; `config/explorerModuleRail.ts` is legacy)
- Explorer control surface tokens for project controls (`config/explorerSurfaces.ts`); exploratory **4px** border radius (`--fd-explorer-controls-surface-border-radius`) is also consumed by account list-element cards, the Reset credentials panel, **`NavigationCard`**, **`.fd-highlight`** / **`Highlight`**, **`CodeBlock`**, **`CodeTabs`**, and the Explorer **Test Request** dialog (UI exploration). Same file: Test Request gutter **`EXPLORER_TEST_REQUEST_MODAL_GUTTER_PX`** (40 — keep in sync with CSS `--fd-explorer-test-request-modal-padding` / `--spacing-250`) and shell-clamp deadband **`EXPLORER_TEST_REQUEST_SHELL_BLOCK_SIZE_UPDATE_THRESHOLD_PX`**
...
- Header/footer brand wordmark typography (`config/brandTypography.ts` — self-hosted Montserrat 700/800 under `public/fonts/montserrat/`; `buildBrandWordmarkFontCss()` + font preloads in `nuxt.config.ts` `app.head`; **not** Google Fonts)
...
- On-this-page TOC thresholds (`config/onThisPageNav.ts` — `ON_THIS_PAGE_NAV_MIN_H2_COUNT`, end-panel min **1280px** / header MenuButton below)
...
- Test wiki base URL + display-name message keys for write-request warning **copy** (`config/wikiInstanceTestWikis.ts` — instance ids match `config/instances.ts`, e.g. `wikidatawiki`; Wikipedia sandbox host `test.wikimedia.org`). Selectability / hide use production-host comparison against Scalar address-bar servers — see `ARCHITECTURE.md` → Write-request production warning
...
- Scalar configuration defaults (`config/scalar.ts` — includes `withDefaultFonts: false` so Inter / JetBrains Mono are not loaded from `fonts.scalar.com`; families remapped to Codex stacks in `main.css`. Also `agent: { disabled: true }` **and** `mcp: { disabled: true }` — both auto-enable on localhost otherwise)
- Scalar interface-localization policy (`config/scalarLocalization.ts` — key prefix, groups excluded from translation, paths pinned to English, and the `ltr` direction pin. The key map itself is generated: `config/generated/scalarLocalization.generated.ts` via `npm run generate-scalar-localization`. Messages live in `i18n/explorer-scalar/`, **never** in `i18n/*.json` — see `docs/adr-scalar-interface-localization.md`)
...
- The endpoint keyword-search index (one record per REST API operation, each with an Explorer deep link) — generated as **phase 3** of that same command into `config/generated/endpointSearchIndex.generated.ts`; consume it through the `config/endpointSearch.ts` policy layer (field weights, match weighting, result cap, snippet length, and `isEndpointSearchable`, which excludes internal-gated endpoints). Never add a separate npm script for it: phase 3 runs in every mode so the index cannot drift from the specs and registry it is derived from (`--index-only` re-runs just that phase). See `docs/adr-explorer-deep-linking.md` §10
- The per-page sidebar map (every page under `content/`, keyed by the route path it is served at, valued at its `sidebar` frontmatter) — **not** committed, unlike everything else here: `modules/content-sidebar-map.mjs` builds it from the frontmatter at build time and again on every content change, and writes `#build/content-sidebar-map`. It exists so `content-sidebar.global` can read one field without making the browser download the content database on first paint; the frontmatter stays the only place a sidebar is declared. Nothing to regenerate and nothing to commit — the builder lives in `scripts/lib/contentSidebarMap.mjs` and is covered by `tests/contentSidebarMap.test.mjs`
- **Node-loadable leaf config.** `config/scalarDocument.ts` (`SCALAR_DOCUMENT_SLUG`) and `config/explorerInstancePolicy.ts` (`QUICK_LINK_INSTANCE_PREFERENCE`, `resolvePreferredModuleInstance`) hold no relative value imports so Node scripts and `node --test` can load them; `config/scalar.ts` and `config/moduleSourceOfTruth.ts` re-export them, so app code imports from the same places as before. Keep them import-free — adding an extensionless relative import to either one breaks the generator and its drift test. This is the same constraint that makes `config/generated/*.generated.ts` self-contained
...
**Documented exception — Test Request shell clamp:** While Test Request is open, scoped styles on `.explorer-page__scalar-shell--client-modal-open` set **`overflow-block: clip`** and **`block-size`** from **`--fd-explorer-test-request-shell-block-size`** so page scroll cannot continue into specs. Unscoped `explorer-codex-overrides.css` cannot beat Vue `[data-v-*]` for those properties — keep the clamp in `app/pages/explorer/[...view].vue`. Overlay/container stay **`height: auto`** (not shell `100%`). On **`scalarReferenceKey`** remount (module / instance / mode change), the explorer page **must** set **`scalarInterface = null`** so `useScalarClientModalBackgroundScrollLock` releases the clamp — Scalar often destroys Test Request without `ui:close:client-modal`, which otherwise left an ~80px (gutter-only) shell. Ignore collapsed dialog measurements (&lt; one gutter). See `ARCHITECTURE.md` → Scalar Test Request modal (natural height).
...
- Use `LinkRow` (`:::link-row`) for **link-only** destination rows under a heading (Contribute by language, Get help, etc.) — not supporting-text-only navigation cards. Each link in its own paragraph; **`--spacing-150` (24px)** column gap. See `ARCHITECTURE.md` → Link row.
...
- Use `CodeBlock` (`:::code-block`) for **every** authored fenced code sample (including URL templates) — same panel chrome as code tabs without the tab header (muted border, exploratory **4px** radius, soft-wrap by default, intentional `dir="ltr"`). Do **not** leave bare fences on hand-authored pages; leave inline `` `code` `` unchanged. Opt out of soft-wrap with `:::code-block{no-soft-wrap}` (horizontal scroll). Fence language must be in `nuxt.config.ts` highlight `langs` (use `bash` / `shell` / `sh` for curl and URL templates — do **not** use `text`; `@shikijs/langs` has no `./text` export and Nuxt Content will fail to resolve it). **Open question (remote import):** whether the wiki HTML→MDC converter should auto-wrap fenced code in `:::code-block` is postponed — see `ARCHITECTURE.md` → Code block / Remote content fetching. See `ARCHITECTURE.md` → “Markdown content pages” → Code block.
...
**One card component, two styles.** Internal and external destination tiles both use `NavigationCard` / `NavigationCardGrid`. Do **not** invent a second **card** component. Choose the style from the destination type. When a human prompt says “internal navigation cards” or “external navigation cards,” follow the matching subsection and copy the MDC shape from the reference pages. When the section only needs plain links (no card title/description), use **`:::link-row`** (`LinkRow`) — that is a layout wrapper, not a card (see **Link-only rows** below).
...
**Do not cardify:** `content/en/get-started/wikimedia-enterprise.md` body sections stay **prose** (heading + paragraphs + writer links). That page may use `::highlight` for the intro CTA only — see `ARCHITECTURE.md` → About Wikimedia Enterprise. Also do **not** cardify link-only rows (see below) — including `/contribute/by-topic#mediawiki`, all project sections on `/contribute/by-language`, and `/get-help`.

**Link-only rows (not cards):** When a section only needs one or more destination links (no title/description card chrome) — e.g. Contribute by language project links, Get help channel links, MediaWiki under Contribute by topic — use `:::link-row` with each Markdown link in its own paragraph. Links get **`--spacing-150` (24px)** horizontal gap via `LinkRow.vue`; vertical rhythm is **`--spacing-100`** with adjoining prose margins zeroed under `.fd-content-page` (same pattern as card grids). Keep writer link labels. Do **not** wrap link-only destinations in `:::navigation-card-grid` / `::navigation-card`. Logo-only repository browsers (`title-logo`) remain cards. Topic overview destination cards on `contribute/by-topic.md` (Localization, Mobile apps, …) stay as external navigation cards — only the MediaWiki link row uses `:::link-row`. References: `content/en/contribute/by-language.md`, `content/en/get-help.md`, MediaWiki section of `content/en/contribute/by-topic.md`.
...
**Prompt phrases (team → agent):** “use link rows”, “remove link-only cards”, “same style as Contribute by language / Get help / Scribe links”, “plain external links with 24px gap”.
...
- Whether a multi-link section should stay as **destination cards** (title + description each) vs a single **`:::link-row`** under one heading
...
See `ARCHITECTURE.md` → Navigation card / Link row and `docs/content-authoring-guide.md` → Navigation cards / Link row.
---
...
- `useEndpointSearch(query)` — REST API endpoint keyword search for the site-wide search panel; runs **alongside** `useContentSearch` (separate indexes, incomparable scores, and endpoint text is English-only so it is not locale-partitioned); lazily `import()`s `config/generated/endpointSearchIndex.generated.ts` on the first query and builds a MiniSearch index from it, cached for the session; that configuration and the snippet builder live in `app/utils/endpointSearch.ts`, policy in `config/endpointSearch.ts`
...
- `useOnThisPageNav(contentRoot, bodyScroll)` — documentation in-page TOC (`h2` + nested `h3`); ≥3 `h2`s; ≥1280px CSS-sticky end panel (`--font-size-small`; not `useEndPanelNavAlign`) / &lt;1280px header MenuButton (`margin-block-end: --spacing-75`); scrollspy progressive active (jump suspends scrollspy until `scrollend` / timeout; near scroll-end activates last heading); banana `on-this-page-*` (not `content-page-nav-label`)
...
- `useScalarClientWriteEndpointWarnings(scalarInterface)` — injects the write-request production **`CdxMessage`** under the Test Request address bar only (select-copy when `isTestWikiSelectableInAddressBar` finds a non-production host in OpenAPI / address-bar servers, otherwise caution; hidden while `isActiveAddressBarServerTestWiki` — active server host ≠ production); strips stray warning hosts elsewhere in the modal
- `useScalarClientWriteRequestConfirmDialog()` — mock Codex confirm before Scalar Send on write methods (`SCALAR_CLIENT_WRITE_REQUEST_CONFIRM_DIALOG_ENABLED`); skipped while `isActiveAddressBarServerTestWiki` (active address-bar host ≠ production)
...
- [ ] Header brand (`ShellHeaderBrand`): banana wordmark + `aria-label` from `app-title`; mark via `WikimediaLogoMark` / `currentColor`; **Montserrat** from self-hosted WOFF2 (`public/fonts/montserrat/`, `config/brandTypography.ts` → `buildBrandWordmarkFontCss()` / `nuxt.config.ts` `app.head` — **no** Google Fonts); label-only warning **Prototype** `CdxInfoChip` after the lockup (`brand-prototype-chip-label`; `--spacing-50` / 8px gap; hide `.cdx-info-chip__icon--vue`; chip outside the home link); Codex `v-tooltip` on chip host (`brand-prototype-chip-tooltip` — “Test prototype. For internal use only”); **no** `:focus` / `:focus-visible` / `:active` / router-active outline on the link (Codex exception #6); first-party CSS uses logical properties
...
- [ ] **On-this-page TOC:** shown on eligible docs routes with ≥3 `h2`s; nesting stops at `h3`; ≥1280px end-column list (**CSS sticky** at column top — not `useEndPanelNavAlign`; end column stretched via `:has(.frontdoor-shell__on-this-page-end)` so sticky engages) with scrollspy **`--color-progressive`** active and **`--font-size-small`**; jump suspends scrollspy until `scrollend`/timeout; near scroll-end activates last heading; &lt;1280px quiet MenuButton opposite primary tabs (`margin-inline-start: auto`, `margin-block-end: --spacing-75`); heading labels in `<bdi>` / `isolatePickerLabel`; UI strings via banana `on-this-page-label` / `on-this-page-nav-aria` (not `content-page-nav-label`); first-party CSS uses logical properties; excluded routes: landing, explorer, account, OAuth callback; **no** primary tab selected on `/account` (or home) — `usePrimaryNavigationTab` must not fall back to Get started
- [ ] Community Explorer endpoint menu is **Scalar’s native sidebar** (`showSidebar` via `EXPLORER_USE_INTERNAL_SCALAR_SIDEBAR`); do not reintroduce the custom end-column module rail as product UX
- [ ] Scalar typography: `withDefaultFonts: false` in `config/scalar.ts` (no `fonts.scalar.com`); `.explorer-page .scalar-app` sets `--scalar-font` → `--font-family-sans-stack` and `--scalar-font-code` → `--font-family-monospace-stack` (Codex tokens — not raw stacks / Inter / JetBrains Mono)
...
- [ ] Scalar Test Request modal write-request **`CdxMessage`**: address-bar only (no request/response ClientPlugin slots; `slotKey === 'address-bar'`; stray hosts under Response Headers removed on scan); banana-i18n for warning + test-wiki names (`explorer-scalar-write-endpoint-warning` when a non-production server is selectable — includes “using the address bar above”; else `explorer-scalar-write-endpoint-warning-no-test-wiki`; plus `explorer-scalar-write-test-wiki-name-*`); **hidden** while the active address-bar server host is not production (`isActiveAddressBarServerTestWiki`) and restored on production; production and test-wiki display names in `<bdi>` as plain text (no link); Front Door does not rewrite URLs (user may select a test server in Scalar); first-party CSS uses logical properties
- [ ] Write-request confirm dialog (`ScalarClientWriteRequestConfirmDialog`): banana keys `explorer-scalar-write-confirm-*`; `$1` production wiki in title (FSI/PDI) and body (`<bdi>`); dialog component is a **sibling** of `#explorer-reference-panel` with `CdxDialog` `target` into that panel; absolute (not viewport-fixed) backdrop; action group **end-aligned** with progressive **Confirm** left of **Cancel** (Codex exception #13); title `--font-size-large` (18px) / body `--font-size-medium` (16px); **skipped** while the active address-bar server host is not production (`isActiveAddressBarServerTestWiki`); Cancel / close does not send; gated by `SCALAR_CLIENT_WRITE_REQUEST_CONFIRM_DIALOG_ENABLED`
...
- [ ] **Link row** (`LinkRow` / `:::link-row`): link-only destinations (not a second card component); each Markdown link in its own paragraph; `column-gap: var(--spacing-150)` (24px); `margin-block: var(--spacing-100)` with adjoining `p`/`ul`/`ol` margins zeroed under `.fd-content-page` in `main.css`; first-party CSS uses logical properties; link labels are page content (not banana-i18n); links via `ProseA` (external icons); demos `contribute/by-language.md`, `get-help.md`, MediaWiki on `contribute/by-topic.md`
- [ ] **Content page typography** (`.fd-content-page` type + spacing): Codex Heading 1 / 2 / 3 on `h1` / `h2` / `h3` per `ARCHITECTURE.md` → Content typography; content-page `h2` **`margin-block-start: --spacing-250` (40px)** (`.fd-content-page` only); **site-wide** `h3` **`margin-block-start: --spacing-200` (32px)** via global `main.css` (not the old `--spacing-150` heading default) — UI chrome `h3`s must keep local margin resets; see `DESIGN_REQUIREMENTS.md` → Content page typography
...
- [ ] **Code block / Code tabs** (`CodeBlock` / `CodeTabs`): every authored fenced sample uses `:::code-block` (or lives inside `::::code-tabs` / `:::code-tab`); inline `` `code` `` unchanged; muted border + exploratory **4px** radius via `--fd-explorer-controls-surface-border-radius` (not hardcoded `4px`); `--spacing-75` padding on `pre`; intentional `dir="ltr"` on `CodeBlock`; soft-wrap long lines by default (`no-soft-wrap` → horizontal scroll); fence language in highlight allowlist (`bash` / `shell` / `sh` — not `text`); keep panel chrome tokens in sync between the two components; remote-import auto-wrap remains an open question
...
- Endpoint browsing uses **Scalar’s native operation sidebar** (`showSidebar: true` via `EXPLORER_USE_INTERNAL_SCALAR_SIDEBAR` / PR #40). The custom end-column **module rail** is not mounted; the explorer end column is collapsed so the reference panel uses the reclaimed width. Project controls keep **`--fd-explorer-controls-surface-*`** (`config/explorerSurfaces.ts`)
...
- Scalar re-renders against the spec URL from discovery when instance (project/language) or REST module select changes; endpoint navigation uses Scalar’s native sidebar
...
- **Write-request production warning (Test Request modal):** for write HTTP methods, a **`CdxMessage`** below the modal address bar (and nowhere else in the modal) warns that the request will modify live data. **Select-copy** (`explorer-scalar-write-endpoint-warning`: “…select to run this endpoint against $2 using the address bar above.”) when OpenAPI / address-bar servers include a non-production host (`isTestWikiSelectableInAddressBar`); otherwise **caution** (`explorer-scalar-write-endpoint-warning-no-test-wiki`). Warning **disappears** when the active address-bar server host is not production (`isActiveAddressBarServerTestWiki`) and **reappears** on production. `$1` / `$2` are plain `<bdi>` text (no link). No URL rewrite by Front Door; no checkbox / no ClientPlugin view slots. Implemented via DOM injection under `.scalar-address-bar` — see `ARCHITECTURE.md` → Write-request production warning
- **Write-request confirm dialog (mock):** before address-bar **Send** on write methods against **production**, a **`CdxDialog`** asks for confirmation (`SCALAR_CLIENT_WRITE_REQUEST_CONFIRM_DIALOG_ENABLED`; easy undo). **Skipped** when the active address-bar server is a non-production host (`isActiveAddressBarServerTestWiki`). Contained to `#explorer-reference-panel`; Confirm left of Cancel in an end-aligned action group (Codex exception #13). See `ARCHITECTURE.md` → Write-request confirm dialog
...
- Write-request Test Request modal: **`CdxMessage`** appears only below the address bar for POST/PUT/PATCH/DELETE (not under Response Headers or other modal sections after Send); select-copy when a non-production server is offered (points at the address bar); caution when not; warning hidden while the active server is non-production; display names in `<bdi>` (no link); Front Door does not rewrite request URLs
- Write-request confirm dialog: address-bar **Send** on write methods against production opens **`CdxDialog`** contained to `#explorer-reference-panel`; **skipped** for non-production address-bar hosts; action group end-aligned with **Confirm** left of **Cancel**; title 18px / body 16px; Cancel does not send; disable via `SCALAR_CLIENT_WRITE_REQUEST_CONFIRM_DIALOG_ENABLED`
