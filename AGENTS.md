...
- `useEndpointSearch(query)` — REST API endpoint keyword search for the site-wide search panel; runs **alongside** `useContentSearch` (separate indexes, incomparable scores, and endpoint text is English-only so it is not locale-partitioned); lazily `import()`s `config/generated/endpointSearchIndex.generated.ts` on the first query and caches it for the session; scoring lives in `app/utils/endpointSearch.ts`, policy in `config/endpointSearch.ts`
