...
- API catalog Wikimedia cards, project-filter options, and filter combobox inline size (`config/apiCatalogWikimedia.ts` — `API_CATALOG_PROJECT_FILTER_COMBOBOX_INLINE_SIZE`)
...
- `useApiCatalogProjectFilter()` — Wikimedia APIs catalog project combobox (banana labels, `isolatePickerLabel` menu items) and visible cards from `config/apiCatalogWikimedia.ts`
