...
- **Approved exception — InfoChips are label-only where documented:** Optional `CdxInfoChip` rows on `NavigationCard` and explorer **API to explore** audience chips (`ExplorerModuleSelectOptionContent`, Codex exception #14) keep stock Codex **status colours** but hide status icons. Codex forces icons on `warning` / `error` / `success` and ignores a null `icon` prop for those statuses, so first-party CSS hides `.cdx-info-chip__icon--vue` (`.navigation-card__chips` / `.explorer-module-select-option__audience-chip`). Do not re-enable icons or invent a second chip component. See `ARCHITECTURE.md` → Navigation card → Info chips and Codex exceptions #14.
