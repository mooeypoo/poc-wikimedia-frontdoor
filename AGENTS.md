...
**Documented exception — landing hero H1 size:** Platform-home hero `h1` uses exploratory **`font-size: 2rem`** (not a Codex `--font-size-*` token; xxx-large is 1.75rem). Intro paragraph size is **`--font-size-x-large`** on `LandingHero.vue` (scoped) — do not redeclare with `:where(p)` in `landing-page.css` (zero specificity loses to the SFC). See `ARCHITECTURE.md` → Platform landing typography.
