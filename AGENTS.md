...
**Read [`DESIGN_REQUIREMENTS.md`](DESIGN_REQUIREMENTS.md) before changing** anything that affects what users see or how they move through the shell: `app/layouts/`, `app/components/shared/`, shell layout CSS (`app/assets/css/page-grid.css`, `app/assets/css/shell-start-nav-reveal.css`, `app/assets/css/shell-start-nav-scroll.css`, `app/assets/css/shell-collapsed-nav-menu.css`, `app/assets/css/shell-end-panel-nav.css`), content-page visual patterns in `app/assets/css/main.css` (e.g. `.fd-content-page` typography and `h2` section spacing), platform landing surfaces in `app/assets/css/landing-page.css` and `app/components/content/Landing*.vue`, explorer UI components (including `app/assets/css/explorer-codex-overrides.css` and scoped shell styles in `app/pages/explorer/[...view].vue` for Test Request), or site-wide visual patterns. Implement to match recorded decisions there (e.g. desktop **4 \| 16 \| 4** grid, community Explorer **Scalar native endpoint sidebar** / collapsed end column per PR #40, Test Request shell clamp / natural-height sandbox, content-page `h2` **`--spacing-250`**, landing content measure **1000px** via `config/landingSurfaces.ts`) unless the user explicitly requests a design change.
