...
Per-language full-text search uses **Nuxt Content FTS5** via `useSearchCollection( 'content' )`. There is no build-time index step; the SQLite FTS5 index is built in the browser. The `useContentSearch( query, activeLocale )` composable handles locale partitioning: results are split into a locale bucket and an English fallback bucket by path prefix on the client side.

- **Index build is deferred to first focus.** `useSearchCollection` defaults to `immediate`, and `useContentSearch` is instantiated from `ShellHeaderUtilityActions.vue` in the default layout, so the default made every route fetch the collection dump and walk every document's AST at mount, on the main thread, for readers who never search. It is now created with `{ immediate: false }`; `loadContentIndex()` starts the build from the search field's `focusin`. Same posture as the endpoint index below.
- **`ensureContentIndex()` owns that build, and every query awaits it.** Not a convenience wrapper: `search()` falls back to `init()` only when its own `db` handle is unset, and the library assigns that handle before the index is built rather than after, so a query issued mid-build runs `collection IN ()` and comes back empty. `init()` also does not dedupe concurrent callers, so two focus events inside one build window would index every section twice. One memoized promise, cleared on failure so the next query retries, covers both. Details in `docs/search-implementation-guide.md`.
