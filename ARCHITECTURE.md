...
| `useShellCollapsedNavMenu({ isNavigationCollapsed, hasSectionNavigation })` | Full-screen collapsed navigation overlay: section vs primary view over `useOverlayDismissal`, whose `onClose` resets the view so Escape and a route change land in the same place as the explicit close |
