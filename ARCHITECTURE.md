...
| `useDeveloperTokenDashboard()` | **Placeholder** API key list state/view-models (not live Meta data), Meta-Wiki request URLs from `config/auth.ts`, locale-aware in-app auth learn-more paths via `resolveContentHref` (same-tab `NuxtLink`), idle Delete handlers; confirm-reset regenerates placeholders via Pinia |
