...
| `useScalarClientWriteEndpointWarnings(scalarInterface)` | Injects write-request production **`CdxMessage`** into the Scalar Test Request modal (DOM mount after `.scalar-address-bar` + `ClientPlugin` slots); `$2` test-wiki link is mocked until test instances are discoverable |
...
- **Sections:** Personal API keys and Application API keys — each section intro is **heading → description → learn-more** (owner-only consumers / OAuth for developers) above the cards. Codex `CdxButton` (quiet Reset / destructive quiet Delete — **idle**, no-op click; progressive “Request new API key”), `CdxMessage` write-token notice on application cards with **`--spacing-75`** (12px) between card content and the message (`AccountOAuthConsumerListItem` stack). List-element cards (`.account-token-list-item__card` via `AccountTokenListItemLayout`) use `--background-color-neutral-subtle` and exploratory **4px** border radius via **`--fd-explorer-controls-surface-border-radius`** from `page-grid.css` / `config/explorerSurfaces.ts` (not a Codex token; Codex `--border-radius-base` is 2px; under consideration as a future system default — same token as explorer project controls / module rail). Interface copy from banana; **placeholder** row fields from `config/tokenManagement.ts` are external (BiDi-isolated).
...
**Account token list UI** (`app/components/account/`): `AccountDeveloperTokenList` / `AccountOAuthConsumerList` render Figma “List-element” cards via `AccountTokenListItemLayout` (header row with title + actions; personal cards show Issued | Status | Permissions; application cards add description, Client ID, masked secret, meta, then write-token notice with **`--spacing-75`** above the `CdxMessage`). Card chrome: `--background-color-neutral-subtle` + exploratory **4px** radius (`--fd-explorer-controls-surface-border-radius`). Secret masking is computed in `useDeveloperTokenDashboard` (`maskSecretValue`), not in the list-item component. `AccountResetApiKeyDialog` wraps Codex `CdxDialog` (confirm then success); success rows use `AccountResetCredentialCopyButton` (quiet `cdxIconCopy` + stable mount + `CdxTooltip`).
...
**Write-request test wiki (Test Request modal).** For write HTTP methods (`POST`, `PUT`, `PATCH`, `DELETE` — `config/scalarWriteHttpMethods.ts`) on wiki instances with a mapped test wiki (`config/wikiInstanceTestWikis.ts`), the explorer injects a production **`CdxMessage`** into Scalar’s Test Request modal. Write requests hit the **production** wiki; there is no checkbox or URL rewrite. The warning’s `$2` test-wiki name is a progressive link that is currently a **mock** (`preventDefault`) until test wikis are selectable via discovery. This is **not** a Scalar `ClientPlugin` alone — the address-bar slot uses **DOM injection** because Scalar’s modal header (`.t-app__top-container`) is flex-wrapped and the warning must sit immediately below the address bar.
...
**Copy and BiDi.** Interface strings use banana-i18n (`explorer-scalar-write-endpoint-warning`, `explorer-scalar-write-test-wiki-name-*`). The warning message splits on `$1` (production wiki display name) and `$2` (test wiki display name such as Test Wikipedia) via `splitMessageAtTwoPositionalParameters()` in `getInterfaceMessageTemplate.ts` so each segment can be wrapped in `<bdi>`; `$2` is rendered as a progressive link (navigation mocked).
