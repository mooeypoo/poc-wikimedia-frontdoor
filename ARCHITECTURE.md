...
Placement uses **DOM injection** after `.scalar-address-bar` (not ClientPlugin `components.request` / `components.response` view slots). Scalar’s response plugin slot renders immediately under **Response Headers** after Send — that path must not show this warning. Scalar's own `mapConfigPlugins` registers only request-hook ClientPlugins — it builds `{ hooks: {} }` and no view components — so there is nothing to suppress; `ScalarClientWriteEndpointWarning` no-ops unless `slotKey === 'address-bar'`; and each modal scan calls `removeStrayWriteWarningHosts()` to drop any host nodes outside `.scalar-client-write-endpoint-warning-mount`.

Until 2026-09 this was enforced by substituting a local copy of Scalar's `map-config-plugins` module via three build-time mechanisms. That override was written against `@scalar/api-reference@^1.46.4` and had silently diverged from upstream (different signature, missing the `onRequestBuilt` hook, and an incompatible `onBeforeRequest` payload contract). It was removed once upstream was confirmed to forward no view components. **If a future Scalar version starts forwarding ClientPlugin view components again, this warning would begin rendering under Response Headers** — re-check `mapConfigPlugins` on upgrades. See `docs/upgrade-plan-dependency-refresh.md` §5.
