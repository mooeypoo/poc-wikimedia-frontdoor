...
│   │   ├── contentSidebarLookup.ts # Route → `sidebar` frontmatter over the generated map (locale fallback)
│   │   └── parseNavigationCardChips.ts # MDC chip attribute → CdxInfoChip props (label-only / award in NavigationCard)
│   ├── middleware/
│   │   └── content-sidebar.global.ts  # Content `sidebar` frontmatter from the generated map; forces `/account` sidebar off
...
│   ├── brandTypography.ts      # Self-hosted Montserrat wordmark faces + CSS builder (not Google Fonts)
...
│   ├── wikiInstanceTestWikis.ts # Test-wiki display-name keys + sandbox base URLs (Wikipedia → test.wikimedia.org); select/hide use production-host comparison
...
├── public/
│   └── fonts/
│       └── montserrat/         # Self-hosted Montserrat WOFF2 700/800 + OFL.txt (brand wordmarks only)
│
...
│   │   └── contentSidebarMap.mjs      # Builds the sidebar map from content/ frontmatter (called by the build, not by hand)
...
│
├── modules/                    # Local Nuxt modules (auto-registered)
│   └── content-sidebar-map.mjs # Writes #build/content-sidebar-map from content/ frontmatter; rebuilds on change
│
├── stores/                     # Pinia stores
...
│
└── nuxt.config.ts              # Nuxt configuration; routeRules; runtimeConfig; brand font head inject
```
...
| `useShellCollapsedNavMenu({ isNavigationCollapsed, hasSectionNavigation, route? })` | Full-screen collapsed navigation overlay: section vs primary view over `useOverlayDismissal`, whose `onClose` resets the view so Escape and a route change land in the same place as the explicit close. `route` is forwarded to `useOverlayDismissal` |
| `useOverlayDismissal({ isCollapsed, onClose?, route? })` | Open state plus the dismissal rules both collapsed-shell overlays share: Escape, route change, and the row that opened it uncollapsing. Owns `isOpen` / `open` / `close` rather than taking them, so the deferred focus trap and focus restore (`DESIGN_REQUIREMENTS.md` → Accessibility (AA)) have one place to capture and restore from. `route` defaults to `useRoute()`; overridable so `tests/overlayDismissal.test.mjs` can exercise the close-on-navigation watch without a Nuxt router context |
...
| `useScalarClientWriteEndpointWarnings(scalarInterface)` | Injects write-request production **`CdxMessage`** into the Scalar Test Request modal **only** after `.scalar-address-bar`; removes stray warning hosts (e.g. under Response Headers); select-copy when a non-production host is among OpenAPI / address-bar servers (`isTestWikiSelectableInAddressBar`), otherwise caution; hidden while the active address-bar server host is not production (`isActiveAddressBarServerTestWiki`) |
| `useScalarClientWriteRequestConfirmDialog()` | **Mock** Codex confirm before Scalar address-bar Send on write methods against production; skipped while `isActiveAddressBarServerTestWiki`; disable via `SCALAR_CLIENT_WRITE_REQUEST_CONFIRM_DIALOG_ENABLED` in `config/scalarClientWriteWarnings.ts` |
...
```
**`sidebar` frontmatter.** A page can override path-based resolution from its frontmatter (`false` collapses the track, a menu id forces that menu). The shell needs that field before the page component loads, so `content-sidebar.global` publishes it from the route. It reads `#build/content-sidebar-map` rather than querying the content collection: global middleware runs in the browser as well as on the server, and any client-side `queryCollection` pulls down the SQLite WASM runtime plus the whole collection dump — 1.95 MB on first paint of every content page, to read one field. `app/utils/contentSidebarLookup.ts` walks the same locale fallback chain against the map that `/api/content-page` walks against the database, stopping at the first locale that has the page whether or not it states a preference. That parity reaches as far as the middleware's own locale derivation: `contentLocaleFromPath` knows the four prefixed locales, so a route under any other registered locale prefix is read as an English slug and resolves to no preference. The database lookup missed the same way, so this is older than the map.

**Where the map comes from.** `modules/content-sidebar-map.mjs` builds it from the markdown at build time, and rebuilds it on any `content/**/*.md` change in dev, so the frontmatter stays the only place a sidebar is declared and nothing has to be regenerated by hand. Pages with no `sidebar` are recorded as `null` rather than omitted — the fallback walk has to stop at the first locale that *has* the page, or a translation that says nothing would inherit English's layout. Keys are file paths, not slugified ones: Nuxt Content slugifies each segment and strips ordering prefixes (`1.intro.md`) and `.draft` suffixes, and rather than reimplement that, the builder refuses to emit a key for a filename that would not survive slugification unchanged.
...
7. **Search field** — `CdxSearchInput` in `ShellHeaderUtilityActions` (`flex: 1 1 auto`, max **40rem**, **256px** min when expanded). `useHeaderUtilityCollapse` (`ResizeObserver` on the utility track) switches to compact mode below `HEADER_UTILITY_COLLAPSE_THRESHOLD_PX` (`config/headerChrome.ts`): search icon, compact language select (icon + code), and `CdxMenuButton` for settings/log in. The collapsed search icon opens **`ShellHeaderSearchOverlay`** (`useShellCollapsedSearchOverlay`): a full-viewport teleported dialog holding the field and `SharedSearchResults`, dismissed by close button, backdrop, Escape, route change, or the utility row uncollapsing (the last three via `useOverlayDismissal`). The icon calls `loadContentIndex()` itself, since there is no `focusin` to start the build from. **One results list at a time:** the expanded search wrap only hides (`v-show`) when collapsed, so its results panel also tests `!isUtilityCollapsed`. Do not drop that condition: the overlay and the hidden panel would each mount a full copy of every result. **Utility option spacing:** `column-gap: var(--spacing-50)` (8px) between options; search → preferences is **`--spacing-100` (16px)** via an extra search-wrap `margin-inline-end`. Brand + utilities share a vertical centerline. **Color theme preferences:** see **Color theme preferences (shell)** below — quiet settings gear (or collapsed menu Settings) opens a content-only `CdxPopover`; former header `CdxToggleButtonGroup` removed.
...
9. **`ShellSiteFooter` wordmark** — **Montserrat** via `--font-family-brand-wordmark` (same self-hosted faces as header — Codex exception #6 / `config/brandTypography.ts`); banana-i18n `brand-wordmark-wikimedia` + **`brand-wordmark-developer-portal`** (shared with header, single horizontal line). Legal body text stays Codex sans.
...
**Codex exceptions:** Footer brand uses **Montserrat** via `--font-family-brand-wordmark` (self-hosted WOFF2 from `config/brandTypography.ts` / Codex exception #6 — **not** Google Fonts; same as header). Footer lockup is **14px mark SVG + composed banana wordmark**, not the Figma **227×14px** horizontal lockup asset (not yet in `public/images/`).
...
| Search icon button (narrow header) | **Implemented** — opens `ShellHeaderSearchOverlay` |
...
| Collapsed hamburger menu overlay | **Implemented** — `useShellCollapsedNavMenu`; `ShellCollapsedNavMenuOverlay`; backdrop-light; `cdxIconPrevious` back; `omitSectionTitleMatching`; **`::after` scroll-end spacer (`--spacing-200`)** |
...
| Collapsed search button activation | **Implemented** — `useShellCollapsedSearchOverlay`; `ShellHeaderSearchOverlay`; backdrop-light; `cdxIconClose` close button; autofocused field |
...
**Dev — corrupt prebundle survives a restart:** A blank explorer with Scalar's `yaml/browser/dist/*` modules stuck pending and `/apis` timing out, often right after a branch switch, is a bad Vite prebundle rather than a code defect. Vite gates re-prebundling on two hashes stored in `node_modules/.cache/vite`: one over **`node_modules/.package-lock.json`** (the installed tree, *not* the repo's `package-lock.json`) and one over `optimizeDeps.include` / `exclude` / `esbuildOptions`, with plugins reduced to their names. Checking out a branch changes neither, since it touches neither `node_modules` nor `nuxt.config.ts`, so Vite trusts whatever is already on disk and restarting `npm run dev` does not help. How the cache goes bad in the first place looks like a dev server killed mid-re-bundle, leaving the deps directory incomplete under a `_metadata.json` that reports it finished (inferred from the symptoms, not proven by a controlled re-run). Clear it with `npm run dev:clean`. Expect ~20s on the first request while it re-bundles.
...
- **Grammar** (community only): `/explorer/direct/<instance>/<module…>#<operation>` (verbose — explicit instance) and `/explorer/q/<module…>#<operation>` (quick — instance resolved from the module source of truth, then canonicalized to the `direct` form). The module name is the multi-segment tail (`site/v1`); the operation anchor's format depends on which component owns the hash (see below). The page is a catch-all route (`app/pages/explorer/[...view].vue`); the grammar is parsed/built by `app/utils/explorerRoute.ts`, and `direct` / `q` join `enterprise` / `enterprise-custom` as reserved first segments.
- **Hash ownership is mode-aware** — with Scalar's internal sidebar on (`EXPLORER_USE_INTERNAL_SCALAR_SIDEBAR`, the shipping build), Scalar's native hash routing is active and Scalar owns the operation hash in its own `#{METHOD}{path}` format; `useExplorerDeepLink` owns the path only, never reads an operation anchor into the intent, and preserves Scalar's hash until the instance or module changes. With the sidebar off (the manual `ExplorerModuleRail`), we own the hash: `useExplorerDeepLink` parses the URL before bootstrap and hands `useExplorerBootstrap` a module/operation intent, and our readable anchor slug (e.g. `#get_v1_page_title`) is resolved against the loaded module's operations (`app/utils/explorerOperationAnchor.ts`) and scrolled via the imperative focus engine. Either way, `scalarReferenceKey` keys on `route.path` (not `route.fullPath`), so writing the operation hash never remounts Scalar (see Reactive spec switching). See `docs/adr-explorer-deep-linking.md` §2; generated search links use Scalar's format, per §10 and **API endpoint search** below.
...
**Write-request production warning (Test Request modal).** For write HTTP methods (`POST`, `PUT`, `PATCH`, `DELETE` — `config/scalarWriteHttpMethods.ts`), the explorer injects a production **`CdxMessage`** **only under the address bar** in Scalar’s Test Request modal. There is no checkbox and no automatic URL rewrite — the user may still send to a sandbox by selecting it in Scalar’s address bar when the OpenAPI document lists it.

**Select vs caution vs hide** (production-host comparison via `isTestWikiSelectableInAddressBar` / `isActiveAddressBarServerTestWiki`, using OpenAPI `servers` plus live address-bar UI URLs from `collectScalarDocumentServerUrls` / `collectScalarAddressBarServerUrls`):

| Condition | Copy / visibility |
|-----------|-------------------|
| Address-bar / OpenAPI servers include a host other than the production wiki (`instances.ts` `baseUrl`) | **Select-copy** — `explorer-scalar-write-endpoint-warning`: “Executing this request will modify live data on $1. To experiment safely, select to run this endpoint against $2 using the address bar above.” (`$2` display name from `getTestWikiDisplayNameMessageKey()` / `config/wikiInstanceTestWikis.ts`) |
| No non-production server offered | **Caution** — `explorer-scalar-write-endpoint-warning-no-test-wiki` |
| Active address-bar server host is not production (e.g. `test.wikimedia.org` while exploring `enwiki`) | Warning **hidden**; confirm-before-Send dialog **skipped**; both **restore** when production is selected again |

Do **not** key select/hide off a brittle config hostname alone — Core REST specs expose `https://test.wikimedia.org` (not `test.wikipedia.org`). Config mapping still supplies banana display names and documented sandbox base URLs. Test-wiki / production names in the message are plain BiDi-isolated text (not links).
...
**Copy and BiDi.** Interface strings use banana-i18n as in the table above. Templates are split with `splitMessageAtTwoPositionalParameters()` / `splitMessageAtFirstPositionalParameter()` in `getInterfaceMessageTemplate.ts` so each substituted value is wrapped in `<bdi>` as plain text (no link). Production display names are external config strings; test-wiki names are interface strings. Instance ids in `wikiInstanceTestWikis.ts` match `config/instances.ts` (e.g. `wikidatawiki`).
...
Per-language full-text search uses **Nuxt Content FTS5**, one collection per `content/<locale>` directory (`content.config.ts`, named via `config/contentCollections.ts`; a hyphenated code like `pt-br` becomes `content_pt_br`, since Nuxt Content collection names must be valid JS identifiers) rather than a single collection covering every locale. The `useContentSearch( query, activeLocale )` composable creates one `useSearchCollection` instance per content locale, each pinned to a **constant** collection name, and searches each locale in a reader's fallback chain (`config/languages.ts`) independently rather than as one combined query. Both choices are load-bearing, not stylistic: pinning each instance to a constant name means nothing about it ever changes, so the library's own internal reactive watcher never fires and each instance's indexed set never grows past its one locale; and searching each locale independently means one locale's search is always scoped to its own rank cutoff, never diluted by every locale a session has touched. Only locales with a `content/<locale>` directory get a collection — most of the language catalog's several hundred interface locales do not, and querying a nonexistent collection name throws rather than returning no results, so both search and page-fallback resolution (`useLocalizedContentPage`) filter a locale's fallback chain down to locales that actually have one before querying.

- **A shared, growing collection list would reintroduce the bug the split exists to fix.** `useSearchCollection`'s `search()` always queries every collection the instance has ever indexed, with no per-call way to narrow that back down; a shared list that only ever grows would mean every search after a reader's second locale (or the all-languages expansion) runs its rank cutoff across every locale touched so far, not just the current one — the exact ranking-starvation bug the split is meant to close.
- **A shared, mutated ref would race the library's own reactivity.** `useSearchCollection` registers `watch(() => toValue(collection), () => init(), { immediate })` internally; mutating a shared ref makes that watcher fire independently of this composable's own `init()` calls, and the two would race — `init()`'s dedupe compares against a value (`indexedFor`) that is only updated once a build finishes, so both would proceed and insert every section twice.
- **Index builds are deferred to first focus, one locale at a time.** `useSearchCollection` defaults to `immediate`, and `useContentSearch` is instantiated from `ShellHeaderUtilityActions.vue` in the default layout, so the default made every route fetch the collection dump and walk every document's AST at mount, on the main thread, for readers who never search. Every instance is created with `{ immediate: false }`; `loadContentIndex()` starts the build from the search field's `focusin`, only for the active locale's fallback chain.
- **`ensureLocaleIndex()` owns each locale's build, and every query awaits it.** Not a convenience wrapper: `search()` falls back to `init()` only when its own `db` handle is unset, and the library assigns that handle before the index is built rather than after, so a query issued mid-build runs against an unindexed collection and comes back empty. `init()` also does not dedupe concurrent callers, so two focus events inside one build window would index every section twice. One memoized promise per locale, cleared on failure so the next query retries, covers both.
- **One locale failing to build or search does not fail the others.** `searchLocales()` catches each locale's build-and-query independently; a locale contributes no results rather than aborting every locale's search, unless every locale in the batch failed, which still surfaces as a genuine "search unavailable."
- **A translated page appears once in the chain, not once per fallback locale.** FTS ids are locale-prefixed paths, so `contentDocumentIdentity()` (`app/utils/contentLocalePaths.ts`) strips the locale segment and the section hash to get what every translation of a page shares, and `dropDuplicateChainDocuments()` keeps only the most-preferred locale's copy. Without it a Brazilian Portuguese reader, whose chain is `pt-br → pt → en` and all three of whose locales have a collection, would see one page listed three times under three headings for any term its translations have in common. Identity is per page, so the dedupe never fires *within* a group: two matching sections of one page are two real hits there and link to different anchors. All-locales mode does not run it at all, since that view is an explicit request for every language.
- **A search that has not settled is not reported as a search that found nothing.** In flight, the panel shows a progress bar; on a failed build, `hasSearchError` renders an error. Either state suppresses the "no results in *X*" messaging, since a query that never finished cannot say whether the locale has matches. Endpoint results answer from their own index and still render alongside both. A rejected FTS expression is *not* covered: `queryFTS` swallows its own SQL errors and returns an empty set, so from outside the library it is indistinguishable from a genuine miss.
...
- **Loading:** a dynamic `import()` on the first query of length ≥ 2, cached for the session — the index is ~16 KB gzipped and is dead weight on pages where nobody searches. MiniSearch rides the same chunk. A failed load degrades to "no endpoint results".
- **Scoring:** `app/utils/endpointSearch.ts` configures MiniSearch: per-field `boost` from the policy layer, `combineWith: 'AND'` so every query token must match, prefix and fuzzy matching weighted below exact hits, and a `boostDocument` penalty that ranks deprecated operations below live ones. Path segments and `operationId` are first-class indexed fields, not decoration: roughly 8% of Wikimedia REST operations ship without a `summary`, and would otherwise be unfindable. The tokenizer is ours rather than MiniSearch's, which leaves symbols glued inside a token.
- **Snippets:** `storeFields` keeps the operation description on the result, so each hit shows a window around the first match with the matched words in `<mark>`, the same treatment the FTS5 content hits get. The upstream text is escaped where the snippet is built, so only those `<mark>` elements reach `v-html`.
...
See `docs/adr-multilingual-search.md` for the full decision record (its §2 "content pages only" is superseded, and its §3 client-side path-prefix partitioning is superseded by the per-locale-collection design above), `docs/search-implementation-guide.md` for background on the library and its dev-cache/connector history (its content-search recipe predates the per-locale collection split and should not be followed for that part), and `docs/adr-explorer-deep-linking.md` §10 for endpoint search.
...
Page and partial lookups go through Nitro routes, not `queryCollection` in the browser. `useLocalizedContentPage()` fetches `/api/content-page`, which walks a reader's whole locale fallback chain in one request; `useContentDocument()` fetches `/api/content-document` for a single known path, which `Include.vue` and `Partial.vue` use. Both routes resolve through `resolveContentDocument` in `server/utils/contentQuerySafety.ts`. `@nuxt/content`'s `executeContentQuery` (`dist/runtime/client.js`) takes its WASM branch wherever `window.WebAssembly` exists and offers no opt-out, so one browser-side `queryCollection` pulled down the SQLite engine and the whole locale dump to read a single document. Page rendering makes no client-side content query at all now. Search is the one exception, below.

`resolveContentDocument` answers a path containing `--` or `/*` as a miss. `@nuxt/content`'s scanner reads either as a SQL comment without noticing it sits inside a quoted literal (`internal/security.js:58-77`) and rejects the query it just built, so the path is unqueryable and a public endpoint should not 500 on it.

**Search still pays for the engine; page rendering no longer does.** `useContentSearch.ts` is the one client-side content query left, through `useSearchCollection` with `{ immediate: false }`, so nothing loads at mount. `loadContentIndex()` starts the build from the search field's `focusin` and from the collapsed icon's click, so focus alone pays for all of it, before a single keystroke. The fetch is 1.27 MB uncompressed over five requests: the wasm binary (856 kB), the `@sqlite.org/sqlite-wasm` glue (200 kB), the search runtime (148 kB), the English dump (63 kB), and the `database.client.js` chunk (3 kB). Page loads and in-app navigations fetch none of it, which is what the routes above bought; verified against a served `.output` on `/`, `/get-started`, `/contribute/search`, `/explorer`, and across eight client-side navigations. Measure this on `node .output/server/index.mjs`, which serves uncompressed, so each response body matches its file in `.output/public` byte for byte. Read the decoded column; the transfer one adds response headers.

Two emitted assets really are dead: the `sqlite3-worker1-bundler-friendly` chunk (197 kB) and a second, byte-identical copy of the wasm. This path loads the engine on the main thread and fetches `sqlite3.DBpDb1lf.wasm` directly, so neither the worker nor `sqlite3-DBpDb1lf.wasm` is ever requested. No module option drops any of it; `content.experimental.sqliteConnector` picks the server connector only.
...
**Locale-relative includes:** `app/components/content/Include.vue` resolves `::include{file="./…"}` against the route directory and the locale the page actually resolved to, then renders via `ContentRenderer`. That locale comes from the page component through `injectResolvedContentLocale` (`app/utils/contentLocaleContext.ts`) and can differ from the interface locale: a German reader on an English fallback page needs the English partial sitting next to it, not a `/de/` path that does not exist. The document itself is fetched through `/api/content-document` (see Server-side content queries above). Demo: `content/en/use-content-and-data.md` → `./_partials/api-note.md`.
...
| Nav collapse + drawer | `app/composables/useShellNavigationCollapse.ts`, `app/composables/useShellNavigationBreadcrumbs.ts`, `app/composables/useShellCollapsedNavMenu.ts`, `app/composables/useOverlayDismissal.ts`, `app/composables/useShellCollapsedSearchOverlay.ts`, `app/components/shared/ShellCollapsedNavigation.vue`, `app/components/shared/ShellCollapsedNavMenuOverlay.vue`, `config/shellNavigation.ts`, `app/assets/css/shell-start-nav-reveal.css`, `tests/overlayDismissal.test.mjs` |
