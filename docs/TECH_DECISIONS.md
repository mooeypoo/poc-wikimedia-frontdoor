...
| Navigation cards | ✅ Implemented | `NavigationCard.vue` + `NavigationCardGrid.vue` — Figma variant A; stretched whole-card link; `supporting-text` progressive link to same `url` (external icon on supporting-text; bottom-aligned in grids); default slot for Markdown description inside grids; content-authored labels in `<bdi>`; see `ARCHITECTURE.md` → Navigation card |
