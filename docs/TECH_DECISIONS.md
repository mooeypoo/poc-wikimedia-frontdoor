...
**Decision:** Warn on write methods in the Scalar Test Request modal; do **not** rewrite requests to a test wiki in this phase. Show a single **`CdxMessage`** under the address bar (DOM injection). Do not use Scalar ClientPlugin `components.request` / `components.response` slots — the response slot mounts under **Response Headers** after Send and duplicated the warning. Select-copy when the address bar offers a non-production host (`isTestWikiSelectableInAddressBar`) — English: “…select to run this endpoint against $2 using the address bar above.”; otherwise caution copy. Hide the warning while the active address-bar server host is not production (`isActiveAddressBarServerTestWiki`); restore it on production. Do not key hide/select off a brittle config hostname alone — Core REST specs use `test.wikimedia.org`, not `test.wikipedia.org`. Test-wiki display names remain banana-i18n.

**Rationale:** Address-bar placement keeps the warning next to the request URL. Matching what Scalar actually lists avoids dead-end “select Test Wikipedia” copy and missed hide behaviour when the sandbox host differs from an older config URL.
...
**Confirm-before-Send mock:** A Codex `CdxDialog` intercepts address-bar Send for write methods against production. Skipped when the active address-bar server host is not production (`isActiveAddressBarServerTestWiki` — same gate as hiding the production warning). Overlay is contained to `#explorer-reference-panel` (Scalar embed; dialog component is a sibling of that panel). Action group stays end-aligned; progressive **Confirm** is left of **Cancel** within the pair (Codex exception #13 — reduces accidental confirms). Title 18px / body 16px. Easy undo: `SCALAR_CLIENT_WRITE_REQUEST_CONFIRM_DIALOG_ENABLED = false` in `config/scalarClientWriteWarnings.ts`.

### Test Request modal — UI exploration (specs ↔ sandbox)

**Decision:** Treat Test Request chrome as a **UI exploration** to improve consulting OpenAPI specs and using the sandbox on the same Explorer page. Specs and the modal both use **natural height**. Clear Scalar viewport caps (`90svh` / `h-dvh` / fill-`100%` / `calc(100% - 120px)`) under `--client-modal-open`. Overlay / container use **`height: auto`** (with `min-height: 100%`) so **`.scalar-app-exit`** dims the reference panel without tying dialog measurement to shell percentage height. Dialog inset by **`--spacing-250` (40px)** (`EXPLORER_TEST_REQUEST_MODAL_GUTTER_PX`); close control keeps Scalar stock size with **4px** (`--spacing-25`) inset from `.scalar.scalar-app` block-start / inline-end. Dialog radius exploratory **4px** via **`--fd-explorer-controls-surface-border-radius`**. While open: (1) clamp shell **`block-size`** to dialog **`scrollHeight` + gutter** (`--fd-explorer-test-request-shell-block-size`, `ResizeObserver`, deadband **`EXPLORER_TEST_REQUEST_SHELL_BLOCK_SIZE_UPDATE_THRESHOLD_PX`**, ignore heights &lt; gutter) with **scoped** **`overflow-block: clip`** so page scroll cannot continue into specs; (2) set the dialog **`inert`** on outside-shell pointerdown so Scalar’s focus-trap cannot dismiss the header preferences popover. On open, scroll the Scalar shell into view; restore shell height + page `scrollTop` on close. On **`ExplorerScalarReference` remount** (`scalarReferenceKey`), null **`scalarInterface`** so the clamp clears even when Scalar skips `ui:close:client-modal` (avoids a stuck ~80px shell after changing **API to explore**). Do **not** freeze `.frontdoor-shell__body-scroll` or viewport-cap the modal. Content above the shell and the footer stay reachable.

**Rationale:** After removing the faux-iframe shell, a viewport-capped modal fought the natural-height page. Exit sized to the dialog box left undimmed spec chrome; Scalar’s in-header close competed with modal content. Without a shell-height clamp, users could scroll past the sandbox into undimmed specs and think they were still reading docs. Clamping to dialog content (not `height: 100%` of the shell) avoids ResizeObserver feedback loops; clamp styles must be **scoped** so they beat `[data-v-*]` `overflow-block: visible`. Spec remount often destroys the modal without a close event — the page must drop the interface handle so clamp CSS cannot stick. Gutter framing and the shared 4px radius keep the sandbox visually related to other Front Door exploratory surfaces. Scalar’s `allowOutsideClick` focus-trap still reclaims focus on `focusin`, which flickered the preferences `CdxPopover` shut — `inert` pauses that reclaim without closing Test Request.

**Source of truth:** `ARCHITECTURE.md` → Scalar Test Request modal (natural height); `DESIGN_REQUIREMENTS.md` → Scalar shell containment → Test Request UI exploration; `AGENTS.md` RTL checklist / config rule 6 (`config/explorerSurfaces.ts`).
...
| Link row | ✅ Implemented | `LinkRow.vue` (`:::link-row`) — horizontal prose links for **link-only** sections (not supporting-text-only cards); `--spacing-150` (24px) column gap; `--spacing-100` block margin; adjoining `p`/`ul`/`ol` margins zeroed under `.fd-content-page` (same pattern as card grids); demos: `contribute/by-language.md`, `get-help.md`, MediaWiki on `contribute/by-topic.md`; see `ARCHITECTURE.md` → Link row |
| Navigation cards | ✅ Implemented | `NavigationCard.vue` + `NavigationCardGrid.vue` — stretched whole-card link; `supporting-text` progressive link to same `url` (external icon; bottom-aligned; **preserve writer labels**); optional label-only `CdxInfoChip` rows (`parseNavigationCardChips`; icons hidden — Codex forces them on warning/error/success; landing `award:` Coolest Tool chips with dark invert via `LANDING_AWARD_CHIP`); default slot for Markdown description inside grids; omit `url` for non-clickable cards; grid **`--spacing-100`** margin-block above/below (adjacent prose margins zeroed); demos include Get started family + **API catalog** (`apis.md`) + platform home (**not** Wikimedia Enterprise body — that stays prose; **not** link-only Contribute/Get help rows — those use `:::link-row`); see `ARCHITECTURE.md` → Navigation card / Link row |
...
| Content page typography | ✅ Implemented | `.fd-content-page` Codex Heading 1 / 2 / 3 type tokens on `h1` / `h2` / `h3`; content-page-only `h2` gap **`--spacing-250` (40px)**; **site-wide** `h3` gap **`--spacing-200` (32px)** via global `main.css` (UI chrome resets locally); decision in `DESIGN_REQUIREMENTS.md` → Content page typography; see `ARCHITECTURE.md` → Content typography |
| Code block | ✅ Implemented | `CodeBlock.vue` — wrap **every** hand-authored fence in `:::code-block`; soft-wrap default, `no-soft-wrap` for horizontal scroll; chrome matches framed code tabs without tabs; demos: landing API curl, Quick start, Authentication, `use-content-and-data.md`; remote-import auto-wrap postponed (open question) |
