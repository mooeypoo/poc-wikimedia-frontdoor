...
/**
 * Quick-link instance policy. Defined in the self-contained
 * `config/explorerInstancePolicy.ts` so the endpoint search index generator can
 * load it from Node and cannot drift from what the runtime resolves; re-exported
 * here so app and server code keep a single import site for the accessor layer.
 */
export { QUICK_LINK_INSTANCE_PREFERENCE, resolvePreferredModuleInstance } from './explorerInstancePolicy'
