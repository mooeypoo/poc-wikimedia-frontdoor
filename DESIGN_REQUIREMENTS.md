...
- **Codex dual-sheet note:** Stacked LTR + RTL Codex stylesheets can still affect **`CdxSelect`** expand indicators elsewhere; the header language control is **`CdxLookup`**, so that chevron issue does not apply here. See `ARCHITECTURE.md` → RTL and BiDi.
...
|--------|---------|-------------|
| *(uncommitted)* | Interface language Lookup popover | Globe + code trigger; popover wraps `CdxLookup`; `visibleItemLimit: 7` / render cap **50**; Floating UI cancel so popover contains menu; flush input–menu spacing |
...
13. **Codex RTL loading strategy** — evaluate alternatives to `link.disabled` toggling (e.g. `codex.style-bidi.css`, disabling LTR base when RTL is active) before production. **May affect** remaining `CdxSelect` expand indicators under stacked LTR + RTL sheets (header language control is now `CdxLookup`; see `ARCHITECTURE.md` → RTL and BiDi).
14. **Remaining `CdxSelect` RTL chevron** — resolve without per-component Codex CSS overrides where dual stylesheets still hide `.cdx-select-vue__indicator`; options include bidi stylesheet or Codex-native trigger icon pattern.
