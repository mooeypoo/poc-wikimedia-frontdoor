...
|--------|---------|-------------|
| *(uncommitted)* | API catalog filter visibility refinements | `excludeProjectIds` on Attribution / Lift Wing / GrowthExperiments; ReadingLists / CampaignEvents / Devices analytics scope → **All projects** |
...
| *(uncommitted)* | API catalog project filter layout | Header `--spacing-150` rhythm; chip↔filter wrap at 24px; combobox width from `API_CATALOG_PROJECT_FILTER_COMBOBOX_INLINE_SIZE`; content `title`/`chip` + banana chrome; static-page island (not `ClientOnly`) |
