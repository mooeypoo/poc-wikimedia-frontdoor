...
| Create an account | Below Log in: banana prompt `account-logged-out-create-account-prompt` (“Don't have a Wikimedia account yet?”) + Codex Link label `account-logged-out-create-account-link` (“Create an account”) to `WIKIMEDIA_CREATE_ACCOUNT_URL` (`config/auth.ts`). New tab; **mock outbound** (no post-registration return). Link chrome from shell `.frontdoor-shell__main a` Codex Link mixin (`--color-link*`); **suppresses `:visited`** (unvisited / hover / active only); text-only (no external icon). Stack gap remains `--spacing-200` |
| Footer | Shell `ShellSiteFooter` at the **bottom of the viewport**; **logged-out gate only** fills remaining vertical space (`AccountLoggedOutGate` flex + `min-block-size: 100%`). Logged-in dashboard is a separate root (see logged-in **Vertical rhythm** below) — never toggle fill styles on the dashboard container. Route is **`ssr: false`** (OAuth memory + handoff). Do **not** duplicate the Figma-embedded footer inside the page |

**Prototype data — not real API keys:** Personal and application key cards and credentials shown after **Reset** are **placeholders for usability testing**. Application list cards show Client ID only (**no** Client secret on the card). They are not retrieved from Meta-Wiki or any live token backend. Reset “regenerates” new placeholder Client secret / Refresh token strings only. **Backend work to list, reset, and revoke real API keys is pending.** See `ARCHITECTURE.md` → Account dashboard → Prototype placeholders. OAuth login (header username / gate Log in) is a real session; that does not make the key tables real. Placeholder rows are seeded only after OAuth login.
...
**i18n:** Interface strings in `i18n/*.json` (`account-*` keys, including `account-logged-out-*`, `account-logged-out-create-account-*`, `account-create-api-token-button`, `account-request-new-oauth-client-button`; Log in button reuses `header-login-label`). Create an account URL and Meta registration URL from `config/auth.ts` (`WIKIMEDIA_CREATE_ACCOUNT_URL`, `META_OAUTH2_CONSUMER_REGISTRATION_URL`). Placeholder row field values in `config/tokenManagement.ts` are external/seed data — BiDi-isolate in templates.
